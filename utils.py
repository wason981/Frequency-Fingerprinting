...
normalize_params = {'cifar10':{'mean':(0.4914, 0.4822, 0.4465),'std':(0.2023, 0.1994, 0.2010)},
...
def denormalize(dataset,batch_data: torch.Tensor):
    transform_reverse = transforms.Compose(
        [
            transforms.Normalize(mean=[0, 0, 0], std=[1 / s for s in normalize_params[dataset]['std']]),
            transforms.Normalize(mean=[-m for m in normalize_params[dataset]['mean']], std=[1, 1, 1]),
        ]
    )
...
def normalize(dataset,batch_data: torch.Tensor):
    normal = transforms.Normalize(mean=normalize_params[dataset]['mean'], std=normalize_params[dataset]['std'])
...
#     #         :]
#     #     kl = part * (np.v1_n_dis_5(part) - np.v1_n_dis_5(np.expand_dims(np.mean(part, axis=0), axis=0)))
...
# images_to_pickle('output/tiny_imagenet/conditional/d0.1alpha1.0beta0.0gamma1.0omega5.0/imgs/320/samples','fingerprint/original/margin_fingerprint.pkl')
